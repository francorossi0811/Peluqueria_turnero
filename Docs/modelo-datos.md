...
| `color` | varchar(7), **null** | HU-34 (15/9/2026): el color con el que Ariel marca **este** turno en la grilla semanal, como `#c0392b`. `null` —lo normal, y lo que quedó en todo lo ya cargado— es "pintalo por su estado". ⚠️ Solo se mira mientras el turno está `reservado`: al marcarlo Realizado o Ausente mandan el verde y el rojo. **No se borra al marcar**, así que sacarle el Ausente a alguien devuelve el turno a su color. Vive acá y no en el navegador por lo mismo que `visto_por_admin`: el color puesto en el celular tiene que verse en la tablet. De paso, los colores recientes salen de esta columna (`ORDER BY updated_at DESC`), sin tabla de preferencias |
