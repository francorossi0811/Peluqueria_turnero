...
- ⚠️ **Los turnos que comparten un rato se reparten el ancho en columnas** (`repartirEnColumnas`), como cualquier calendario. Antes se apilaban en absoluto y el de abajo desaparecía por completo. **Que dos turnos se pisen NO es un error de datos y no hay que "arreglarlo" en el backend:** marcar Ausente libera el rato que queda —el cliente no vino a los 10 minutos, Ariel lo marca y mete a otro— y el `EXCLUDE` de la base, limitado a `WHERE estado = 'reservado'`, es exactamente esa regla escrita donde corresponde. La primera lectura de este bug fue la contraria (endurecer el `EXCLUDE` a los tres estados que la agenda dibuja) y **habría roto justo el flujo que Ariel usa**; queda anotado porque el código *parece* incoherente —la agenda muestra tres estados y la disponibilidad cuenta uno— y no lo es. Los turnos que no se pisan entre sí comparten columna, así el grupo no se angosta de más, y uno solo devuelve `columnas: 1` con los mismos 2 px de cada lado que daba `left-0.5 right-0.5`: **sin solapamiento la grilla queda idéntica a como estaba**. Medido en el DOM sobre columna de 235 px: turno solo 2/230, pisados 2/113 y 119/113.
